...
# Contributing
First off all, you're going to need to have [Github Desktop](https://github.com/apps/desktop?ref_cta=download+desktop&ref_loc=installing+github+desktop&ref_page=docs) installed. 
Then once opened go to ```File``` then select ```Clone repository```, after that choose ```URL``` where you paste in ```https://github.com/Kravataf/GDXR``` to clone the repository so you can do PRs or other stuff..
